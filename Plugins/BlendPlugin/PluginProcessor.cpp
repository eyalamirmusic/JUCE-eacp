...
juce::AudioProcessorValueTreeState::ParameterLayout Processor::makeParameters()
{
    ...
    // Read only by the editor, and only to hand to the eacp view. The audio
    // thread never looks at it, which is why nothing here smooths it.
    layout.add(std::make_unique<juce::AudioParameterFloat>(
...
{
    return new BlendPlugin::Processor();
}
